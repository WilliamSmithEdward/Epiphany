...
4. **ADR-0004, Embedded TypeScript engine.** QuickJS versus V8 (`deno_core`) versus WASM (`wasmtime`); the transpile pipeline (swc); the sandbox and resource-limit model. Decided with a performance spike. *(Phase 5, Accepted: [docs/adr/0004-embedded-javascript-engine.md](adr/0004-embedded-javascript-engine.md) - chose pure-Rust boa with an in-house type stripper.)*
5. **ADR-0005, Automatic feeder inference strategy.** What is statically derivable, how manual feeds are validated, and how under-feeding and over-feeding are detected. *(Phase 4, Accepted: [docs/adr/0005-automatic-feeder-inference.md](adr/0005-automatic-feeder-inference.md).)*
...
7. **ADR-0007, Rule evaluation strategy.** Compile rules to bytecode or closures (no per-cell re-parsing); the evaluation plan, memoization, and invalidation; an optional JIT later. *(Phase 4, Accepted: [docs/adr/0007-rule-evaluation-strategy.md](adr/0007-rule-evaluation-strategy.md).)*
...
- **Delivered (M4).** Rules grammar (the contract the parser and the canonical `Display` share, round-trippable): an ordered list of `area = formula ;` statements; an area is `['Dim': selector, ...]` over per-dimension selectors (a named element, `{leaves}`, `{consolidated}`, `{all}`, `{children of 'x'}`, `{descendants of 'x'}`, or an `@'attr' op literal` predicate), with an absent dimension unconstrained; a formula is arithmetic (`+ - * /`, unary minus), `IF cond THEN x [ELSE y]` with `AND`/`OR`/`NOT` and `= <> < <= > >=`, cell references (`value[...]` same-cube, `'Cube'![...]` cross-cube), and a closed function allow-list (`Attr`, `AttrNum`, `IsLeaf`, `ElementName`, `Undef`). **Area-overlap precedence:** rules are matched in source (author) order; the first matching rule for a coordinate wins, and `{all}`/an unconstrained dimension matches leaves only, so a measure rule rolls up while an explicitly-named consolidated element acts as a consolidation override. **Numerics:** exact scaled-integer `Fixed` throughout, with round-half-to-even on `*` and `/` (ADR-0008); rule evaluation is numeric-only for M4 (string formulas and text functions parse but are deferred). **Feeders, honest scope (ADR-0005):** the dense `consolidate_with` is the always-correct read path, so a read is never wrong because of feeders; sparse feeders are inferred for the statically analyzable leaf rule and validated against the dense truth (under-feed is the hard error, over-feed a warning with an estimated byte cost), while a rule that cannot be localized (constant, or cross-cube/consolidated-driven only) is reported opaque rather than guessed. **Evaluation (ADR-0007):** rules compile once per version to a resolved, index-addressed AST (no per-cell re-parsing) and evaluate on-demand with a per-query memo keyed by `(cube, coord)` and precise self-reference cycle detection; cross-cube references must address every dimension explicitly (relative `with (src -> dst)` mapping is parsed but deferred). The calc layer depends on core only and reaches values through a core-owned `CellResolver` seam, so `epiphany-engine` stays calc-free and the API composition root injects a rule-aware `CalcFactory`. **Provenance:** `explain` returns a depth-bounded trace whose value always agrees with the evaluator and whose nodes distinguish a stored leaf, a firing rule (with its source span), and a consolidation. **Model-as-code and testing:** rules and rule unit tests serialize in the durable model (checkpoint-on-define, no WAL change); `run_rule_tests` clones the cube, applies fixtures, and checks cell assertions deterministically. **Web:** a per-cube rules workspace (editor with debounced server-side validation and located error markers, a feeder-diagnostics panel, an explain tree, and a test runner); it follows the M3 textarea + live-validation pattern, so the web app stays dependency-free and Monaco syntax highlighting is the one deferred piece. **Dependency-free tests:** the calc engine is covered by hand-written golden assertions and `DeterministicRng`-seeded property loops (children sum to parent under weights, sparse-fed equals dense, identical results run to run) rather than `insta`/`proptest`, keeping the workspace `cargo-deny` green. Gated by `epiphany-api/tests/m4_acceptance.rs`. See [docs/adr/0005-automatic-feeder-inference.md](adr/0005-automatic-feeder-inference.md) and [docs/adr/0007-rule-evaluation-strategy.md](adr/0007-rule-evaluation-strategy.md).
...
- **Delivered (M5).** **Engine choice (ADR-0004):** the embedded engine is pure-Rust **boa**, chosen over QuickJS/V8/WASM by a build spike because it links into the single static binary with no C step on the GNU toolchain, is fully determinism-controllable, and the script layer only orchestrates (raw JS speed does not matter). **TypeScript:** rather than pull a ~350-crate transpiler (oxc/swc) just to strip types, a focused dependency-free **type stripper** (`epiphany-flow::strip`, in the spirit of the hand-written MDX/rules lexers) removes type syntax and is conservative and fail-loud (an unclassifiable construct is left for the parser to reject, never silently corrupted); it blanks types to spaces so error lines map back to source. Type checking is editor-side via a shipped `.d.ts`; the supported subset is documented and exotic constructs (enums, namespaces, decorators, inline object/function types in annotation position, imports) are rejected. **Programming model:** a flow declares top-level `init`/`schema`/`rows`/`finalize` functions called with a `ctx`; the host API is vectorized (`ctx.input()` returns all rows; `ensureElements`/`addChild` build members; `writeCells` stages writes) so the script is never on the per-cell hot path. **Building members:** dimensions, previously immutable, gain an append-only, idempotent runtime growth path (`Cube::extend_schema`) that preserves existing element indices and re-packs cells only when a dimension's bit-width grows (the durable model-text is name-based, so recovery is layout-independent). **Determinism and sandboxing (ADR-0009):** each run uses a fresh context with the wall-clock global removed and the RNG made to throw; `ctx.now()` is the injected clock; there is no filesystem or network (only the host API is exposed); a loop-iteration and recursion budget caps runaway scripts deterministically instead of a wall-clock timeout; numeric values cross the boundary as exact strings (never `f64`, ADR-0008). **Pure runner:** `epiphany-flow` depends on core only and returns a `FlowOutcome` the API applies through the engine (elements first, then cells), so the engine stays flow-free and a flow is unit-testable without a server. **Data sources:** flat/CSV (a small in-house RFC-4180 reader), source-less, and a guided CSV import wizard that builds members and loads values without a hand-written script. **Model-as-code and testing:** flows and flow tests serialize as durable `[[flow]]`/`[[flow_test]]` text (checkpoint-on-define, backward-compatible byte-identical when absent); `run_flow_tests` extends the model testing framework deterministically. **Web:** a per-cube flows workspace (validating editor, run-over-CSV, the import wizard, and a test runner), dependency-free like the rules workspace. **Honest deferrals:** the **SQL data source** (needs an external DB driver, against the dependency discipline) and the **job scheduler** (Phase 8) are deferred - the DoD binds on CSV and on-demand running; Monaco syntax highlighting and a hard memory cap are likewise deferred (the CPU budget bounds runaway scripts). Gated by `epiphany-api/tests/m5_acceptance.rs`. See [docs/adr/0004-embedded-javascript-engine.md](adr/0004-embedded-javascript-engine.md).
- **Delivered (post-M5): data-source connectors (ADR-0012).** Flows can ingest from outside the model through admin-defined **connections**, on a fetch/transform split: the impure fetch runs at the edge (a new `epiphany-connect` crate) and produces the same rows `ctx.input()` consumes, so the flow engine, sandbox, and determinism are unchanged (a flow test still pins inline rows; there is no `ctx.fetch` in the script). The first connector is **command** - run a program and read its stdout (CSV or JSON) - the single mechanism behind ingest-from-a-Python-script, a-PowerShell-script, or an-exe. Because that is host code execution, it is fenced by four controls: admin-defined fixed commands only (no flow-supplied args), spawned directly with an argv array (no shell, no injection), off unless the server opts in (`EPIPHANY_ENABLE_COMMAND_CONNECTORS`), and resource-bounded (timeout, stdout cap, non-zero-exit error). Connections serialize as durable `[[connection]]` model-as-code; the flow run body takes an optional `connection` source; the web flows workspace gains a source picker and a connection admin panel. An **HTTP fetch connector and a secret store** for its credentials shipped as the follow-on on this foundation (ADR-0030): off by default, host-allowlisted (SSRF-bounded), credentials referenced by name from an owner-only store so they never enter the model. **Databases (including ODBC) are reached through a command connection running the user's own client script** rather than a built-in connector, so the server keeps no DB driver and stays a single pure-Rust binary (ADR-0012 decision 7). Gated by `epiphany-api/tests/connectors.rs`. See [docs/adr/0012-data-source-connectors.md](adr/0012-data-source-connectors.md).
...
- **Extension (m8.2, ADR-0016):** global (all-cubes) grants and **explicit deny** for cube access, resolved by most-specific-tier-wins with deny-wins-in-tier and absolute admin bypass, so a broad baseline ("read every cube") with per-cube exceptions ("deny this one", "write that one") is expressible for users and groups, on top of the closed-by-default posture (m8.1).
