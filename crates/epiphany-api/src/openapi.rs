...
//! numbers (ADR-0008).
//!
//! The document is composed from per-area builders (`auth_paths`, `cube_paths`,
//! `dimension_paths`, `view_paths`, `security_paths`, `flow_paths`,
//! `connection_paths`, `job_paths`, plus `schemas`) so each section stays
//! independently readable. `document()` merges the per-area path maps into one
//! `paths` object; the served document is byte-for-byte the same as the previous
//! single-literal form.
...
/// Merge each per-area path map into `paths`. Every area returns a JSON object
/// whose keys are path strings; the areas are disjoint, so a duplicate key would
/// be a bug (the coverage test asserts the full set).
fn merge_paths(into: &mut Map<String, Value>, area: Value) {
    if let Value::Object(map) = area {
        for (path, item) in map {
            let existed = into.insert(path.clone(), item).is_some();
            debug_assert!(!existed, "duplicate OpenAPI path: {path}");
        }
    }
}
...
fn document() -> Value {
    ...
    merge_paths(&mut paths, meta_paths());
    merge_paths(&mut paths, auth_paths());
    merge_paths(&mut paths, cube_paths());
    merge_paths(&mut paths, dimension_paths());
    merge_paths(&mut paths, view_paths());
    merge_paths(&mut paths, flow_paths());
    merge_paths(&mut paths, job_paths());
    merge_paths(&mut paths, connection_paths());
    merge_paths(&mut paths, security_paths());
...
/// Unauthenticated meta endpoints: liveness and the OpenAPI document itself.
fn meta_paths() -> Value {
    json!({
        "/healthz": { "get": {
...
/// Session lifecycle and the current principal.
fn auth_paths() -> Value {
    json!({
...
/// Cube lifecycle, structural editing of a cube's own dimensions, attributes,
/// cell read/write/spread, rules, feeders, MDX/cellset, and what-if sandboxes.
fn cube_paths() -> Value {
    json!({
        "/api/v1/cubes": {
            "get": {
...
/// The shared dimension library (ADR-0024) and registry-level structural edits.
fn dimension_paths() -> Value {
    json!({
        "/api/v1/dimensions": {
            "get": {
...
/// Subsets, saved views, and ad-hoc cellset/MDX-preview resolution.
fn view_paths() -> Value {
    json!({
...
/// Flows, flow tests, flow preview, and guided CSV import (ADR-0035).
fn flow_paths() -> Value {
    json!({
        "/api/v1/flows": { "get": {
...
/// Scheduled jobs and the run ledger (ADR-0013/0035).
fn job_paths() -> Value {
    json!({
...
/// Data-source connections (ADR-0027/0030/0034).
fn connection_paths() -> Value {
    json!({
...
/// The admin surface: users, groups, element ACLs, modular grants, the
/// server-wide overview, secrets, and the audit query.
fn security_paths() -> Value {
    json!({
        "/api/v1/users": {
            "get": {
...
/// The `components` object: the security scheme plus every request/response
/// schema referenced by `$ref` above.
fn components() -> Value {
    json!({
...
/// Every request/response schema, composed from cohesive sub-builders (the same
/// disjoint-merge pattern as the paths). Splitting keeps each `json!` literal
/// small enough that the crate compiles at the default macro recursion limit.
fn schemas() -> Value {
    let mut map = Map::new();
    merge_paths(&mut map, common_schemas());
    merge_paths(&mut map, cell_schemas());
    merge_paths(&mut map, structural_schemas());
    merge_paths(&mut map, view_schemas());
    merge_paths(&mut map, automation_schemas());
    merge_paths(&mut map, security_schemas());
    Value::Object(map)
}

/// Shared envelopes referenced across areas.
...
/// Auth and cell-data request/response bodies.
fn cell_schemas() -> Value {
    json!({
...
/// Cube creation and structural-edit request bodies.
fn structural_schemas() -> Value {
    json!({
...
/// Subset, view, MDX, rules, and rule-test schemas.
fn view_schemas() -> Value {
    json!({
...
/// Flow, flow-test, job, connection, import, and sandbox schemas.
fn automation_schemas() -> Value {
    json!({
        ...
        "Connection": { "type": "object", "properties": {
            "name": { "type": "string" },
            "kind": { "type": "string", "enum": ["command", "http", "sql"], "description": "command (ADR-0012), http (ADR-0030), or sql (ADR-0034); each needs its server opt-in" },
            ...
            "url": { "type": "string", "description": "Fetch URL (http kind, ADR-0030)" },
            "auth": { "type": "object", "description": "http credential referencing a secret by name", "properties": { "kind": { "type": "string", "enum": ["bearer", "basic"] }, "secret": { "type": "string" } } },
            "engine": { "type": "string", "enum": ["postgres", "mysql"], "description": "SQL engine (sql kind, ADR-0034)" },
            "host": { "type": "string", "description": "Database host (sql kind; must be allowlisted)" },
            "port": { "type": "integer", "description": "Database port (sql kind)" },
            "database": { "type": "string", "description": "Database name (sql kind)" },
            "user": { "type": "string", "description": "Database user (sql kind)" },
            "password_secret": { "type": "string", "description": "Name of the secret holding the password (sql kind; never the value)" },
            "query": { "type": "string", "description": "The SQL query to run (sql kind)" },
            "ssl_mode": { "type": "string", "enum": ["verify-full", "require", "disable"], "description": "TLS mode (sql kind, ADR-0034)" } },
            "required": ["name", "kind"] },
...
/// User, group, element-ACL, and modular-grant schemas.
fn security_schemas() -> Value {
    json!({
...
mod tests {
    ...
    /// The document parses as a valid OpenAPI 3.1 object and still carries a
    /// representative path from each per-area builder, with its expected method
    /// and a 200 (or other 2xx) response. Guards the split against silently
    /// dropping or restructuring an area.
    #[test]
    fn document_contains_a_representative_path_per_area() {
        let doc = document();
        assert_eq!(doc["openapi"], "3.1.0");
        let paths = doc["paths"].as_object().unwrap();

        // (path, method) -> a representative entry per area.
        let representatives = [
            ("/healthz", "get"),                         // meta
            ("/api/v1/auth/login", "post"),              // auth
            ("/api/v1/cubes/{cube}/cells/read", "post"), // cube
            ("/api/v1/dimensions", "get"),               // dimension
            ("/api/v1/cubes/{cube}/views", "get"),       // view
            ("/api/v1/flows", "get"),                    // flow
            ("/api/v1/schedules", "get"),                // job
            ("/api/v1/connections", "get"),              // connection
            ("/api/v1/acl/grants", "get"),               // security
        ];
        for (path, method) in representatives {
            let item = paths
                .get(path)
                .unwrap_or_else(|| panic!("missing path {path}"));
            assert!(
                item.get(method).is_some(),
                "path {path} is missing method {method}"
            );
            let responses = item[method]["responses"].as_object().unwrap();
            assert!(
                responses.keys().any(|code| code.starts_with('2')),
                "path {path} {method} has no 2xx response"
            );
        }

        // Component schemas survived the extraction.
        let schemas = doc["components"]["schemas"].as_object().unwrap();
        for schema in ["Error", "Coord", "LoginRequest", "Grant", "Connection"] {
